...
function getLeastDepth(str: string) {
  let leastDepth: number = 0;
  ...
  for (const l of lines) {
    ...
    if (leastDepth === undefined || depth < leastDepth) leastDepth = depth;
  }

  return leastDepth;
}

/** Function to check if line in YAML text is base line. and return matching key in load. */
function handleLine(
  line: string,
  str: string,
  load: Record<string, unknown>
): { isBaseLine: true; key: string };
function handleLine(
  line: string,
  str: string,
  load: Record<string, unknown>
): { isBaseLine: false; key: undefined };
function handleLine(
  line: string,
  str: string,
  load: Record<string, unknown>
): { isBaseLine: boolean; key: string | undefined } {
  let lineDepth = 0;
  ...
  if (lineDepth > 0) {
    ...
    if (lineDepth > leastDepth) return { isBaseLine: false, key: undefined };
  }
...
export const onHover = vscode.languages.registerHoverProvider(
  { scheme: "file", language: "yaml" },
  {
    provideHover(document, position, token) {
      ...
      if (!isObject(loadCache)) return null;
      ...
      const { isBaseLine, key } = handleLine(
        line,
        str,
        loadCache as Record<string, unknown>
      );
      if (!isBaseLine || !key) return null;
      ...
      // get value to show
      const value = (loadCache as Record<string, unknown>)[key];
