...
export function handleThisPath(
  filename: string,
  segments: string[],
  position: vscode.Position
): vscode.CompletionList<vscode.CompletionItem> | null {
  // copy segments so we don't mutate the caller's array
  const segs = [...segments];

  // get cache of the file
  let current = getModule(filename, true) as Record<string, any> | null;
...
export function handleImportPath(
  filename: string,
  segments: string[],
  position: vscode.Position
): vscode.CompletionList<vscode.CompletionItem> | null {
  ...
  if (segs.length === 1) {
    ...
    current = Object.fromEntries(
      Array.from(importsMap.keys()).map((v) => [v, undefined])
  ...
  const alias = segs.shift() as string; // safe because segs.length >= 1

  const targetFile = cache.directives?.importsMap.get(alias as string)?.path;
  ...
  current = getModule(resTargetFilePath, false) as Record<string, any> | null;
  if (!current) return null;
...
export function handleThisLocals(
  filename: string,
  segments: string[],
  position: vscode.Position
): vscode.CompletionList<vscode.CompletionItem> | null {
...
export function handleImportParamsExpr(
  filename: string,
  segments: string[],
  position: vscode.Position,
  exprBase: string
