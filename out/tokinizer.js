 ...
 */
export function divideByDelimiter(str, delimiter, startIdx) {
...
export function splitAtDelimiter(str, delimiter, startIdx) {
    /** Var to hold parts. */
    let parts = [];
 ...
 */
export function getNextDelimiter(str, delimiter, startIdx) {
    /** Function to check for delimiter */
    const delimiterFunc = getDelimiterFunc(delimiter);
...
function getDelimiterFunc(delimiter) {
    if (delimiter === " ")
        return (ch) => WHITE_SPACE_REGEX.test(ch);
    else
 ...
 */
export function getNextChar(str, startIndex, closeChar) {
    let i = startIndex + 1;
