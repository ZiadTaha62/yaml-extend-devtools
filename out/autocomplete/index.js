...
import { handleImportParamsExpr, handleImportPath, handleLocal, handleParam, handleThisLocals, handleThisPath, } from "./expressions.js";
...
export const onDollarAutoComplete = vscode.languages.registerCompletionItemProvider({ scheme: "file", language: "yaml" }, {
    provideCompletionItems(document, position, token, context) {
        ...
        if (dollarMatch) {
            ...
            const items = EXPR_BASES.filter((b) => b.startsWith(partial)).map((b) => {
                const item = new vscode.CompletionItem(b, vscode.CompletionItemKind.Variable);
...
export const onDotAutoComplete = vscode.languages.registerCompletionItemProvider({ scheme: "file", language: "yaml" }, {
    provideCompletionItems(document, position, token, context) {
        if (document.languageId !== "yaml")
            return null;
        ...
        if (dotDollarMatch) {
            ...
            if (pathStr.startsWith("."))
                pathStr = pathStr.slice(1); // remove starting "."
            ...
            if (base === "local")
                return handleLocal(filename, pathStr, position);
            if (base === "param")
                return handleParam(filename, pathStr, position);
...
export const onSpaceAutoComplete = vscode.languages.registerCompletionItemProvider({ scheme: "file", language: "yaml" }, {
    provideCompletionItems(document, position, token, context) {
        if (document.languageId !== "yaml")
            return null;
        const filename = document.fileName;
        ...
        if (spaceDollarMatch) {
            ...
            if (segments.length < 2)
                return null; // at least starting $this... or $import... then a space (2 segments)
            // get expr base
            const exprBase = segments.shift();
...
export const onPercentAutoComplete = vscode.languages.registerCompletionItemProvider({ scheme: "file", language: "yaml" }, {
    provideCompletionItems(document, position, token, context) {
        if (document.languageId !== "yaml")
            return null;
        const line = document.lineAt(position.line).text;
        ...
        if (percentMatch) {
            ...
            const startChar = position.character - partial.length;
            const replaceRange = new vscode.Range(position.line, startChar, position.line, position.character);
            const items = DIR_BASES.filter((b) => b.startsWith(partial)).map((b) => {
                const item = new vscode.CompletionItem(b, vscode.CompletionItemKind.Variable);
                item.range = replaceRange;
                item.insertText = b + " ";
                return item;
